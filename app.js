...
app.post('/allurldata', function(req, res){
  db.urldata.find({}).skip(0).sort({_id: -1}).toArray(function (err, urldata) {
    res.send(urldata);
  });
});

app.post('/sendresultinemails', function(req, res){
...
app.post('/removeurl', function(req, res){
  var id = req.body.id;
  var ObjectID = require('mongodb').ObjectID;
...
app.post('/declineAccessRequest', function(req, res){
  var id = req.body.id;
  var ObjectID = require('mongodb').ObjectID;
  var o_id = new ObjectID(id);
...
app.post('/savepdfs', function(req, res){       
...
function pdfparser(name, path, portfoliourl, originalFilename, timestamp){
  ...
  pdfParser.on("pdfParser_dataReady", pdfData => {
    ...
    var email = extractEmails(data); if(email===null || email==='') email = "Update..";
    var phone = extractPhone(data); if(phone===null || phone==='') phone = "Update..";
    var newPortfolio = {postedby: name,url: portfoliourl, title: data.substring(0, 50), body: data, email: email, phone: phone, comment: "No comments yet", stars: 0}
...
app.post('/uploaddp', requireLogin ,function(req, res){       
  var file = req.files.file;
  var filepath = file.path;
  var id = req.body.id;
  var Client = require('ftp'); 
...
app.get('/savebookmark/:key', requireLogin, function(req, res){   
...
app.post('/bookmarkthisportfolio/', requireLogin, function(req, res){       
  var url = req.body.url;
  console.log(url);
...
app.post('/deletethisportfolio', requireLogin, function(req, res){       
  var url = req.body.url;
...
app.post('/deleteportfoliobookmark', requireLogin, function(req, res){       
  var id = req.body.id;
  var ObjectID = require('mongodb').ObjectID;
  var o_id = new ObjectID(id);
...
app.get('/deleteallportfolios', requireLogin, function(req, res){       
  db.urldata.remove();
    res.redirect("/");
});

app.post('/uploaddbug', requireLogin ,function(req, res){   
  var date = new Date();
...
app.post('/changebugstatus', requireLogin ,function(req, res){   
...
app.post('/saveLinkedInBulkSite', function(req, res){       
  var dataoflink = "";
  var bulkurl = req.body.bulkurl;
  ...
  for(i=0; i<urlarray.length; i++){
    ...
    console.log("CONSOLE INSIDE FOR LOOP: LinkedinName = "+linkedinname+", URL: "+urlarray[i]);
...
function saveLinkedInSite(linkedinname, uploadername, url){
   scrapper.fetch(linkedinname).then(function(profilee, err){
    ...
    body = body.split('\\').join(" ");
    body = body.split('"').join("");
    console.log("Came in saveLinkedinProfile method with name: "+ uploadername+", "+linkedinname)
